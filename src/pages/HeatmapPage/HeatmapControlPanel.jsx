    ...
    return (
        <div className="flex gap-4 bg-gray-100 p-4 rounded-lg flex-wrap">
            ...
            <div className="flex flex-col justify-between bg-white px-4 py-3 rounded shadow-sm w-[270px] h-[80px]">
                ...
                <div>
                    <input
                        ...
                        className="w-full"
                    />
                    <span className="text-xs text-gray-400 mt-1 block">피어슨 (Pearson) 기준</span>
                </div>
            </div>

            {/* 3. 정규화 기준 */}
            <div className="flex flex-col justify-between bg-white px-4 py-3 rounded shadow-sm w-[270px] h-[80px]">
                <span className="text-sm text-gray-600 font-medium">정규화 기준</span>
                <div className="flex gap-x-3 gap-y-1 flex-wrap">
                    {["z-score", "%Change", "Min-Max", "백분위(percentile)"].map((opt) => (
                        <label key={opt} className="flex items-center gap-1 text-sm cursor-pointer">
                            <span
                                className={`w-2.5 h-2.5 rounded-full ${normalization === opt ? "bg-black" : "bg-gray-300"
                                    }`}
                            ></span>
                            <span>{opt}</span>
                            <input
            ...
            <div className="flex flex-col justify-between bg-white px-4 py-3 rounded shadow-sm w-[200px] h-[80px]">
                <span className="text-sm text-gray-600 font-medium">수익률 기준</span>
                <div className="flex gap-3">
                    {["당일", "±1일", "±3일"].map((opt) => (
                        <label key={opt} className="flex items-center gap-1 text-sm cursor-pointer">
                            <span
                                className={`w-2.5 h-2.5 rounded-full ${returnWindow === opt ? "bg-black" : "bg-gray-300"
                                    }`}
                            ></span>
                            <span>{opt}</span>
                            <input
