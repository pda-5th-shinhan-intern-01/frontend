...
const sectors = [
  { sectorName: "기술", description: "소프트웨어, 하드웨어, 반도체, IT 서비스를 포함하는 섹터", stocks: ["AAPL", "MSFT", "NVDA", "ORCL"] },
  { sectorName: "금융", description: "은행, 보험, 자산관리, 금융 서비스를 포함하는 섹터", stocks: ["JPM", "BAC", "WFC", "BRK.B"] },
  { sectorName: "헬스케어", description: "제약, 바이오테크, 의료기기, 의료서비스를 포함하는 섹터", stocks: ["LLY", "JNJ", "UNH", "ABBV"] },
  { sectorName: "에너지", description: "석유, 가스, 석탄, 신재생 에너지 등을 포함하는 섹터", stocks: ["XOM", "CVX", "COP", "WMB"] },
  { sectorName: "자유소비재", description: "자동차, 의류, 미디어, 호텔, 레저 등 선택적 소비재 섹터", stocks: ["AMZN", "TSLA", "HD", "MCD"] },
  { sectorName: "필수소비재", description: "식품, 음료, 가정용품, 개인용품 등 필수 소비재 섹터", stocks: ["WMT", "COST", "KO", "PEP"] },
  { sectorName: "커뮤니티", description: "통신, 미디어, 엔터테인먼트, 소셜 미디어 등을 포함하는 섹터", stocks: ["META", "GOOG", "NFLX", "TMUS"] },
  { sectorName: "산업재", description: "항공우주, 방위산업, 건설, 기계, 운송 등을 포함하는 섹터", stocks: ["GE", "UBER", "RTX", "DE"] },
  { sectorName: "유틸리티", description: "전기, 가스, 수도, 재생에너지 등 공공 서비스 섹터", stocks: ["NEE", "SO", "DUK", "CEG"] },
  { sectorName: "부동산", description: "부동산 투자 신탁(REITs), 부동산 개발, 관리 등을 포함하는 섹터", stocks: ["AMT", "PLD", "WELL", "EQIX"] },
  { sectorName: "소재", description: "화학, 금속, 광업, 종이, 포장재 등을 포함하는 섹터", stocks: ["LIN", "SHW", "ECL", "NEM"] },
];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {sectors.map((sector, idx) => {
          ...
          const matching = changeRates.find((item) => item.sectorName === sector.sectorName);
              ...
              <div className="flex flex-col h-full justify-between">
                <div>
                  <div className="mb-2 flex justify-between">
                    <h2 className="text-3xl font-semibold mb-2">{sector.sectorName}</h2>
                    <div
                      className="text-lg flex justify-center items-center px-4 rounded-full font-semibold"
                      style={{ backgroundColor: changeBgColor, color: changeTextColor }}
                    >
