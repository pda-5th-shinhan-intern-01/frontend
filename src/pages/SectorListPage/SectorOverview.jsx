...
const sectors = [
    ...
    },
    {
        name: '부동산',
        change: '+5.55%',
        description: '부동산 투자 신탁(REITs), 부동산 개발, 관리 등을 포함하는 섹터',
        stocks: ['JNJ', 'PFE', 'MRK', 'MRK'],
        more: 8,
    },
    {
...
export default function SectorOverview({ onSelectSector }) {
    return (
        <div className="bg-gray-200 p-5">
            ...
            <p className="mb-8 text-gray-700">11개 주요 시장 섹터와 각 섹터에 포함된 대표 종목들을 확인하세요.</p>
                    ...
                    <div key={idx} className="bg-white rounded-xl shadow p-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {sector.stocks.map((stock, sIdx) => (
                                <span key={sIdx} className="bg-gray-200 px-3 py-1 text-sm rounded-full">{stock}</span>
                            ))}
                            <span className="bg-gray-300 px-3 py-1 text-sm rounded-full">+{sector.more}</span>
                        </div>
                    </div>
                ))}
            </div>
